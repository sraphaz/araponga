...
public sealed class UserActivityService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var purchases = checkouts
            .Where(c => c.Status == Arah.Modules.Marketplace.Domain.CheckoutStatus.Paid)
        ...
        var count = checkouts.Count(c => c.Status == Arah.Modules.Marketplace.Domain.CheckoutStatus.Paid);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var sales = allCheckouts
            .Where(c => storeIds.Contains(c.StoreId) && c.Status == Arah.Modules.Marketplace.Domain.CheckoutStatus.Paid)
            .OrderByDescending(c => c.CreatedAtUtc)
            .Skip(pagination.Skip)
            .Take(pagination.Take)
        ...
        const int maxInt32 = int.MaxValue;
        var count = allCheckouts.Count(c => storeIds.Contains(c.StoreId) && c.Status == Arah.Modules.Marketplace.Domain.CheckoutStatus.Paid);
        var totalCount = count > maxInt32 ? maxInt32 : count;
