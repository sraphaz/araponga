...
public sealed class PostCreationService
{
        ...
        IReadOnlyCollection<string>? tags = null)
    {
        ...
        ArahMetrics.PostsCreated.Add(1, new KeyValuePair<string, object?>("territory_id", territoryId));

        return Result<CommunityPost>.Success(post);
    }

    private static IReadOnlyCollection<Arah.Modules.Map.Domain.PostGeoAnchor> BuildPostAnchors(
        Guid postId,
        IReadOnlyCollection<Models.GeoAnchorInput>? geoAnchors)
    {
        if (geoAnchors is null || geoAnchors.Count == 0)
        {
            return Array.Empty<Arah.Modules.Map.Domain.PostGeoAnchor>();
        }
        ...
        return geoAnchors
            ...
            .Select(anchor => new Arah.Modules.Map.Domain.PostGeoAnchor(
                Guid.NewGuid(),
                postId,
                anchor.Latitude,
                anchor.Longitude,
