...
public sealed class PostgresUserConnectionRepository : IUserConnectionRepository
{
    ...
    public PostgresUserConnectionRepository(ArapongaDbContext dbContext)
    {
    ...
    {
        var record = await _dbContext.UserConnections
            .AsNoTracking()
    ...
    {
        var records = await _dbContext.UserConnections
            .AsNoTracking()
    ...
    public Task<UserConnection> AddAsync(UserConnection connection, CancellationToken cancellationToken)
    {
        ...
        _dbContext.UserConnections.Add(record);
        return Task.FromResult(connection);
    }

    public async Task UpdateAsync(UserConnection connection, CancellationToken cancellationToken)
    {
        var record = await _dbContext.UserConnections.FirstOrDefaultAsync(c => c.Id == connection.Id, cancellationToken);
