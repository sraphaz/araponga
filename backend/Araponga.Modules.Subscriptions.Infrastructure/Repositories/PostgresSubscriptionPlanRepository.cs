...
namespace Araponga.Modules.Subscriptions.Infrastructure.Repositories;
...
public sealed class PostgresSubscriptionPlanRepository : ISubscriptionPlanRepository
{
    ...
    public PostgresSubscriptionPlanRepository(SubscriptionsDbContext dbContext)
    {
    ...
    public async Task UpdateAsync(SubscriptionPlan plan, CancellationToken cancellationToken)
    {
        ...
        record.CapabilitiesJson = JsonSerializer.Serialize(plan.Capabilities);
        record.LimitsJson = plan.Limits != null ? JsonSerializer.Serialize(plan.Limits) : null;
