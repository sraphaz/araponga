...
namespace Araponga.Domain.Work;
...
public sealed class WorkItem
{
        ...
        string? completionNotes)
    {
        ...
        if (normalizedSubjectType.Length > MaxSubjectTypeLength)
        {
            throw new ArgumentException($"SubjectType must not exceed {MaxSubjectTypeLength} characters.", nameof(subjectType));
        }
        ...
        if (normalizedNotes is not null && normalizedNotes.Length > MaxNotesLength)
        {
            throw new ArgumentException($"Completion notes must not exceed {MaxNotesLength} characters.", nameof(completionNotes));
        }

        // Se exige capability territorial, territoryId precisa existir
        if (requiredCapability.HasValue && territoryId is null)
        {
            throw new ArgumentException("TerritoryId is required when RequiredCapability is set.", nameof(territoryId));
        }
    ...
    public void Complete(WorkItemOutcome outcome, Guid completedByUserId, string? notes, DateTime completedAtUtc)
    {
        ...
        {
            throw new InvalidOperationException("Work item is cancelled.");
        }

        var normalizedNotes = string.IsNullOrWhiteSpace(notes) ? null : notes.Trim();
        if (normalizedNotes is not null && normalizedNotes.Length > MaxNotesLength)
        {
            throw new ArgumentException($"Completion notes must not exceed {MaxNotesLength} characters.", nameof(notes));
        }
    ...
    public void Cancel(Guid cancelledByUserId, string? notes, DateTime cancelledAtUtc)
    {
        ...
        if (Status == WorkItemStatus.Cancelled)
        {
            throw new InvalidOperationException("Work item is already cancelled.");
        }

        var normalizedNotes = string.IsNullOrWhiteSpace(notes) ? null : notes.Trim();
        if (normalizedNotes is not null && normalizedNotes.Length > MaxNotesLength)
        {
            throw new ArgumentException($"Cancellation notes must not exceed {MaxNotesLength} characters.", nameof(notes));
        }
