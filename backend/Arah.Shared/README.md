...
Não coloque aqui:
- Entidades de domínio → **Arah.Domain** ou no módulo.
- Interfaces de aplicação ou repositórios → **Arah.Application** (Interfaces).
- Código específico de um módulo → no próprio módulo.

## Referências

Este projeto deve ter **poucas dependências** (idealmente só .NET BCL) para ser referenciado sem ciclos. **Arah.Application** referencia Arah.Shared.
