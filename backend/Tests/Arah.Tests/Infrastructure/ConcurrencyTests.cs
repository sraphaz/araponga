...
public sealed class ConcurrencyTests : IClassFixture<DatabaseFixture>
{
    ...
    public async Task UpdateCommunityPost_ThrowsConcurrencyException_WhenRowVersionMismatch()
    {
        ...
        // Para executar: export TEST_DATABASE_CONNECTION_STRING="Host=localhost;Database=Arah_test;Username=postgres;Password=postgres"
        ...
        // Arrange
        var options = new DbContextOptionsBuilder<ArahDbContext>()
        ...
        // Criar post inicial
        using (var context1 = new ArahDbContext(options))
        {
        ...
        using (var context2 = new ArahDbContext(options))
        {
        ...
        using (var context3 = new ArahDbContext(options))
        {
...
public sealed class DatabaseFixture : IDisposable
{
    ...
    public DatabaseFixture()
    {
        ...
        ConnectionString = Environment.GetEnvironmentVariable("TEST_DATABASE_CONNECTION_STRING")
            ?? "Host=localhost;Database=Arah_test;Username=postgres;Password=postgres";
    }
    ...
    public async Task<bool> IsDatabaseAvailableAsync()
    {
        try
        {
            ...
            using var context = new ArahDbContext(options);
