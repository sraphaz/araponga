...
using Arah.Api.Contracts.Chat;
using Arah.Api.Contracts.Feed;
using Arah.Api.Contracts.Media;
...
using Arah.Tests.Api;
using Microsoft.Extensions.DependencyInjection;
using Reqnroll;
using Xunit;

namespace Arah.Tests.Api.BDD;
...
public sealed class MediaSteps
{
    ...
    public Task GivenQueExisteUmTerritorio(string territoryName)
    {
        ...
        var featureFlagService = _factory.Services.GetRequiredService<Arah.Application.Interfaces.IFeatureFlagService>();
    ...
    public async Task GivenQueExisteUmUsuarioComoResidente(string userName)
    {
        ...
        {
            var userResponse = await meResponse.Content.ReadFromJsonAsync<Arah.Api.Contracts.Users.UserProfileResponse>();
            if (userResponse is not null)
            {
                ...
                if (user is not null)
                {
                    user.UpdateIdentityVerification(Arah.Domain.Users.UserIdentityVerificationStatus.Verified, DateTime.UtcNow);
                }
    ...
    public async Task GivenQueExisteUmaImagemDeMBDisponivel(int sizeMB)
    {
        ...
        if (originalAsset is not null && sizeMB > 5) // Apenas para imagens maiores que o limite de chat
        {
            ...
            if (originalAsset.SizeBytes < desiredSizeBytes)
            {
                ...
                var newAsset = new Arah.Domain.Media.MediaAsset(
                    originalAsset.Id,
    ...
    {
        if (_scenarioContext.TryGetValue<Arah.Api.Contracts.Events.EventResponse>("createdEvent", out var eventResponse))
        {
            ...
            if (userProfile is not null)
            {
                ...
                if (membership is not null)
                {
                    ...
                    if (settings is null)
                    {
                        ...
                        settings = new Arah.Domain.Membership.MembershipSettings(
                            membership.Id,
    ...
    {
        if (_scenarioContext.TryGetValue<Arah.Api.Contracts.Marketplace.ItemResponse>("createdItem", out var item))
        {
    ...
    private async Task CreateEventWithMediaAsync(Guid coverMediaId, Guid[]? additionalMediaIds)
    {
        _lastResponse = await _client!.PostAsJsonAsync(
            ...
            new Arah.Api.Contracts.Events.CreateEventRequest(
                _currentTerritoryId!.Value,
                "Evento de teste BDD",
                "Descrição do evento",
        ...
        if (_lastResponse.StatusCode == HttpStatusCode.Created)
        {
            var eventResponse = await _lastResponse.Content.ReadFromJsonAsync<Arah.Api.Contracts.Events.EventResponse>();
    ...
    private async Task SendChatMessageWithMediaAsync(Guid mediaId)
    {
        ...
        if (!_scenarioContext.TryGetValue<Guid>("conversationId", out conversationId))
        {
            ...
            {
                var channels = await channelsResponse.Content.ReadFromJsonAsync<Arah.Api.Contracts.Chat.ListChannelsResponse>();
