...
public sealed class DevicesControllerTests
{
    ...
    public async Task RegisterDevice_WhenValid_CreatesDevice()
    {
        ...
        var token = await AuthTestHelper.LoginForTokenAsync(client, "google", externalId);
        Assert.False(string.IsNullOrEmpty(token), "Token should not be empty");

        using var scope = factory.Services.CreateScope();
        var tokenService = scope.ServiceProvider.GetRequiredService<Arah.Application.Interfaces.ITokenService>();
        ...
        AuthTestHelper.SetupAuthenticatedClient(client, token, $"devices-test-session-{Guid.NewGuid()}");
    ...
    public async Task ListDevices_WhenValid_ReturnsDevices()
    {
        ...
        AuthTestHelper.SetupAuthenticatedClient(client, token, sessionId);
