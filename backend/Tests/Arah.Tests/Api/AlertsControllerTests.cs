...
public sealed class AlertsControllerTests
{
    ...
    private static async Task<string> LoginForTokenAsync(HttpClient client, string provider, string externalId)
    {
        var response = await client.PostAsJsonAsync(
            "api/v1/auth/social",
            new Arah.Api.Contracts.Auth.SocialLoginRequest(
                provider,
                externalId,
                "Test User",
                "123.456.789-00",
        ...
        var payload = await response.Content.ReadFromJsonAsync<Arah.Api.Contracts.Auth.SocialLoginResponse>();
    ...
    public async Task GetAlertsPaged_ReturnsPagedResults()
    {
        ...
        {
            var result = await response.Content.ReadFromJsonAsync<Arah.Api.Contracts.Common.PagedResponse<AlertResponse>>();
