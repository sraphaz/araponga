...
**Arquivo**: `backend/Arah.Tests/appsettings.json`
...
**Local:** A implementação compartilhada está em **Arah.Tests.ApiSupport** (`AuthTestHelper` e `BaseApiFactory`). O Core e o módulo Subscriptions referenciam ApiSupport; no Core, `Arah.Tests.TestHelpers.AuthTestHelper` é um facade que repassa para ApiSupport (compatibilidade).
...
Módulos sem projeto de teste dedicado (Feed, Events, Notifications, Chat, Alerts) são cobertos por **Arah.Tests** (integração e serviços). Para adicionar testes específicos do módulo, criar **Arah.Tests.Modules.\<Nome\>** seguindo o padrão de Connections ou Subscriptions.
...
**ApiSupport:** O projeto **Arah.Tests.ApiSupport** centraliza `BaseApiFactory` (env vars: JWT, RateLimiting, Persistence, InMemory) e `AuthTestHelper`. Arah.Tests e Arah.Tests.Modules.Subscriptions usam essa base; assim a configuração de testes fica em um só lugar. Ver ADR-019.

## Estrutura de Testes (por projeto)

- **Arah.Tests**: testes do **Core** — Api, Application, Domain, Infrastructure, Bff, Performance. Projeto principal.
- **Arah.Tests.Shared**: compartilhado entre projetos de teste (ex.: `TestIds`). Referenciado por Core e por testes de módulos.
- **Arah.Tests.Modules.Connections**: testes do módulo **Connections** — Domain, Application e fluxos de notificação.
- **Arah.Tests.Modules.Moderation**: testes do módulo **Moderation** — Domain e Application (DocumentEvidence, WorkQueue, Verification, ReportCreatedWorkItem).
- **Arah.Tests.Modules.Marketplace**: testes do módulo **Marketplace** — Domain e Application (Cart, Store, Inquiry, Rating, PlatformFee, SellerPayout, MarketplaceSearch, MarketplaceService, TerritoryPayoutConfig).
- **Arah.Tests.Modules.Subscriptions**: testes do módulo **Subscriptions** — Application, Api (integração) e Performance. Outros módulos podem ganhar `Arah.Tests.Modules.*` no futuro.
