...
using Xunit;
using static Arah.Tests.Shared.TestIds;
...
public sealed class AuthService2FATests
{
    ...
    public async Task Setup2FAAsync_GeneratesSecretAndQR()
    {
        ...
        var result = await service.Setup2FAAsync(UserId, "user@Arah.com", CancellationToken.None);
        ...
        Assert.Contains("Arah", result.Value.QrCodeUri);
    }
    ...
    public async Task Confirm2FAAsync_ValidatesCode()
    {
        ...
        // Setup
        var setupResult = await service.Setup2FAAsync(UserId, "user@Arah.com", CancellationToken.None);
