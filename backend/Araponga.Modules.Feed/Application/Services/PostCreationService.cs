...
/// <summary>
/// Service responsible for creating posts in the Feed module.
/// </summary>
        ...
        IReadOnlyCollection<string>? tags = null)
    {
        ...
        if (_accessEvaluator is not null)
        {
            ...
            if (policiesResult.IsFailure || !policiesResult.Value)
            {
                ...
                return Result<Post>.Failure(errorMessage);
            }
        }

        if (string.IsNullOrWhiteSpace(title) || string.IsNullOrWhiteSpace(content))
        {
            return Result<Post>.Failure("Title and content are required.");
        }

        // Converter tipos do módulo para tipos antigos para validação
        var oldType = (Araponga.Domain.Feed.PostType)(int)type;
        if (oldType == Araponga.Domain.Feed.PostType.Alert && !_featureFlags.IsEnabled(territoryId, Models.FeatureFlag.AlertPosts))
        {
            return Result<Post>.Failure("Alert posts are disabled for this territory.");
        }
        ...
        if (mapEntityId is not null)
        {
            ...
            {
                return Result<Post>.Failure("Map entity not found for territory.");
            }
        }
        ...
        {
            return Result<Post>.Failure("User is restricted from posting in this territory.");
        }
        ...
        if (normalizedAssetIds is not null && normalizedAssetIds.Count > 0)
        {
            ...
            {
                return Result<Post>.Failure("Asset not found for territory.");
            }
        }
        ...
        if (normalizedMediaIds is not null && normalizedMediaIds.Count > 0)
        {
            ...
            {
                return Result<Post>.Failure("One or more media assets not found.");
            }

            // Validar que todas as mídias pertencem ao usuário
            if (mediaAssets.Any(media => media.UploadedByUserId != userId || media.IsDeleted))
            {
                return Result<Post>.Failure("One or more media assets are invalid or do not belong to the user.");
            }
        ...
        {
            // Criar post temporário para validação (usando tipo antigo)
            var tempPost = new CommunityPost(
                ...
                content,
                oldType,
                (Araponga.Domain.Feed.PostVisibility)(int)visibility,
                (Araponga.Domain.Feed.PostStatus)(int)status,
                mapEntityId,
                DateTime.UtcNow,
                tags: tags?.ToList());
            ...
            {
                return Result<Post>.Failure(moderationResult.Error ?? "Post violates territory moderation rules.");
            }
        }

        // Criar post usando entidade do módulo
        var post = new Post(
            Guid.NewGuid(),
            territoryId,
            userId,
        ...
        // Converter para CommunityPost temporariamente para persistência
        ...
        await _feedRepository.AddPostAsync(communityPost, cancellationToken);
        ...
        if (normalizedAssetIds is not null && normalizedAssetIds.Count > 0)
        {
            var postAssets = normalizedAssetIds.Select(assetId => new Araponga.Domain.Feed.PostAsset(post.Id, assetId)).ToList();
